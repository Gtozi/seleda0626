-- Migration 182: Kitchen Management Module — DB Functions
-- Recipe costing with unlimited nesting, production completion, FEFO consumption,
-- POS integration, expiry alerts, waste processing, production planning.

-- ── 1. Recursive recipe cost calculation (supports unlimited nesting) ───
CREATE OR REPLACE FUNCTION public.calculate_kitchen_recipe_cost(
  p_recipe_id UUID,
...
BEGIN
  SELECT * INTO v_recipe FROM public.kitchen_recipes WHERE id = p_recipe_id;
  IF NOT FOUND THEN RETURN; END IF;

  -- Recursively calculate cost of all ingredient lines
  WITH RECURSIVE ingredient_tree AS (
    -- Base: direct ingredients of this recipe
    SELECT
      kri.id,
      kri.recipe_id,
      kri.ingredient_type,
      ...
      kri.waste_percent,
      kri.unit,
      kri.cost_at_time_of_costing,
      1 AS depth
    FROM public.kitchen_recipe_ingredients kri
    WHERE kri.recipe_id = p_recipe_id

    UNION ALL

    -- Recursive: ingredients of sub-recipes
    SELECT
      kri.id,
      kri.recipe_id,
      kri.ingredient_type,
      kri.ingredient_id,
      kri.raw_ingredient_id,
      (kri.quantity * it.quantity * (1 + it.waste_percent / 100))::numeric(12,3),
      kri.waste_percent,
      kri.unit,
      kri.cost_at_time_of_costing,
      it.depth + 1
    ...
    WHERE it.ingredient_type = 'sub_recipe'
    ...
    COALESCE(SUM(
      CASE
        ...
        WHEN it.ingredient_type IN ('sub_recipe','finished_product') AND it.ingredient_id IS NOT NULL THEN
          it.quantity * (1 + it.waste_percent / 100) * COALESCE(it.cost_at_time_of_costing, kr.total_cost / NULLIF(kr.yield_qty, 0), 0)
        ELSE 0
      END
    ), 0),
  ...
  LEFT JOIN public.ingredients ing ON it.raw_ingredient_id::text = ing.id
  LEFT JOIN public.kitchen_recipes kr ON it.ingredient_id = kr.id;
...
COMMENT ON FUNCTION public.calculate_kitchen_recipe_cost(UUID, NUMERIC) IS
  'Recursively calculates recipe cost including nested sub-recipes with unlimited depth';

-- ── 2. Update recipe cost (trigger-friendly, callable) ──────────────────
CREATE OR REPLACE FUNCTION public.update_kitchen_recipe_cost(
  p_recipe_id UUID
) RETURNS VOID AS $$
DECLARE
  v_cost NUMERIC;
...
BEGIN
  ...
  FROM public.kitchen_recipes WHERE id = p_recipe_id;

  SELECT total_cost, cost_per_portion INTO v_cost, v_cost_per_portion
  FROM public.calculate_kitchen_recipe_cost(p_recipe_id, COALESCE(v_yield, 1));

  v_food_cost_pct := CASE WHEN v_selling_price > 0 AND v_cost_per_portion > 0
  ...
  SET total_cost = COALESCE(v_cost, 0),
      ...
      food_cost_percent = COALESCE(v_food_cost_pct, 0)
  WHERE id = p_recipe_id;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- ── 3. FEFO batch consumption ───────────────────────────────────────────
-- Consumes quantity from batches using First-Expired-First-Out by default
CREATE OR REPLACE FUNCTION public.consume_kitchen_batch_fefo(
...
DECLARE
  ...
  v_order_by TEXT;
BEGIN
  -- Determine sort order based on consumption method
  ...
  -- Loop through batches in FEFO/FIFO order
  FOR v_batch IN
    EXECUTE format(
      'SELECT * FROM public.kitchen_inventory_batches
  ...
  LOOP
    ...
    UPDATE public.kitchen_inventory_batches
  ...
  IF v_remaining > 0 THEN
    RAISE EXCEPTION 'Insufficient inventory for item %, short by %', p_inventory_item_id, v_remaining;
  END IF;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

COMMENT ON FUNCTION public.consume_kitchen_batch_fefo(UUID, NUMERIC, TEXT, TEXT, TEXT, TEXT, UUID) IS
  'Consumes inventory from batches using FEFO (default) or FIFO, recording movements';

-- ── 4. Complete production order ────────────────────────────────────────
-- Deducts raw materials, calculates actual cost, creates inventory batch
CREATE OR REPLACE FUNCTION public.complete_kitchen_production(
...
DECLARE
  ...
  v_expiry_date DATE;
BEGIN
  SELECT * INTO v_order FROM public.kitchen_production_orders WHERE id = p_production_order_id;
  IF NOT FOUND THEN RAISE EXCEPTION 'Production order not found'; END IF;

  SELECT * INTO v_recipe FROM public.kitchen_recipes WHERE id = v_order.recipe_id;
  ...
  FOR v_line IN SELECT * FROM public.kitchen_production_lines WHERE production_order_id = p_production_order_id LOOP
    IF v_line.actual_qty > 0 THEN
      -- Find or create kitchen inventory item for this ingredient
      IF v_line.ingredient_type = 'raw_material' AND v_line.raw_ingredient_id IS NOT NULL THEN
        SELECT id INTO v_inventory_item_id FROM public.kitchen_inventory_items
      ...
      IF v_inventory_item_id IS NOT NULL THEN
        ...
        v_consumed_cost := 0;
        FOR v_consumed_qty IN
          SELECT quantity_consumed FROM public.consume_kitchen_batch_fefo(
        ...
        -- Update production line cost
        UPDATE public.kitchen_production_lines
  ...
  v_batch_number := 'KB-' || TO_CHAR(NOW(), 'YYMMDD') || '-' || SUBSTRING(p_production_order_id::TEXT, 1, 6);

  -- Calculate expiry
  v_shelf_life := COALESCE(v_recipe.portion_size, 0); -- placeholder; could use recipe shelf_life field
  v_expiry_date := CURRENT_DATE + COALESCE(v_shelf_life, 7); -- default 7 days

  -- Create inventory batch for produced item
  -- Find or create kitchen inventory item for the produced recipe
  SELECT id INTO v_inventory_item_id FROM public.kitchen_inventory_items
  ...
  -- Create batch
  INSERT INTO public.kitchen_inventory_batches (
    ...
    chef_id, storage_location_id, status
  ) VALUES (
    v_order.property_id, v_inventory_item_id, v_batch_number, v_order.recipe_id, p_production_order_id,
    v_order.production_date, v_expiry_date, COALESCE(v_shelf_life, 7),
    p_actual_qty, p_actual_qty, v_cost_per_unit, v_total_cost,
    v_order.chef_id, v_order.storage_location_id, 'active'
  ) RETURNING id INTO v_batch_id;

  -- Record production movement (IN)
  INSERT INTO public.kitchen_inventory_movements (
    inventory_item_id, batch_id, movement_type, direction,
    quantity, unit, unit_cost, total_cost,
    reference_type, reference_id, performed_by, to_location_id
  ...
  UPDATE public.kitchen_production_orders
  SET actual_qty = p_actual_qty,
      yield_percent = v_yield_pct,
  ...
  -- Audit log
  INSERT INTO public.kitchen_audit_log (
...
COMMENT ON FUNCTION public.complete_kitchen_production(UUID, NUMERIC, TEXT, NUMERIC) IS
  'Completes a production order: deducts raw materials, calculates cost, creates inventory batch, records variance';

-- ── 5. POS sale integration — consume ingredients from recipe ───────────
CREATE OR REPLACE FUNCTION public.consume_kitchen_for_pos_sale(
  p_menu_item_id UUID,
  p_quantity NUMERIC,
  p_performed_by TEXT DEFAULT NULL,
  p_reference_id TEXT DEFAULT NULL
...
DECLARE
  ...
  v_consumed_cost NUMERIC;
BEGIN
  -- Find the kitchen recipe linked to this menu item
  SELECT id INTO v_recipe_id FROM public.kitchen_recipes
  WHERE recipe_type = 'menu_item' AND status = 'active'
    AND recipe_code = (SELECT name FROM public.pos_menu_items WHERE id = p_menu_item_id)
  LIMIT 1;

  -- Fallback: try matching by name in kitchen_recipes
  IF v_recipe_id IS NULL THEN
    SELECT kr.id INTO v_recipe_id
    FROM public.kitchen_recipes kr
    WHERE kr.recipe_type = 'menu_item' AND kr.status = 'active'
  ...
  -- Consume each ingredient line
  FOR v_line IN
    SELECT * FROM public.kitchen_recipe_ingredients
  ...
  LOOP
    ...
    IF v_inventory_item_id IS NOT NULL THEN
      v_consumed_cost := 0;
      SELECT SUM(total_cost) INTO v_consumed_cost
      FROM public.consume_kitchen_batch_fefo(
        v_inventory_item_id,
...
COMMENT ON FUNCTION public.consume_kitchen_for_pos_sale(UUID, NUMERIC, TEXT, TEXT) IS
  'Automatically consumes kitchen inventory when a menu item is sold via POS';

-- ── 6. Expiry alerts view ───────────────────────────────────────────────
CREATE OR REPLACE VIEW public.kitchen_expiry_alerts AS
SELECT
  bib.id AS batch_id,
  bib.batch_number,
  kii.id AS inventory_item_id,
  ...
  bib.production_date,
  bib.expiry_date,
  bib.best_before_date,
  bib.shelf_life_days,
  bib.storage_location_id,
  ksl.name AS storage_location_name,
...
FROM public.kitchen_inventory_batches bib
JOIN public.kitchen_inventory_items kii ON bib.inventory_item_id = kii.id
LEFT JOIN public.kitchen_storage_locations ksl ON bib.storage_location_id = ksl.id
CROSS JOIN (SELECT expiry_alert_days FROM public.kitchen_settings LIMIT 1) ks
WHERE bib.remaining_qty > 0
  AND bib.status = 'active'
  AND bib.is_deleted = false
ORDER BY bib.expiry_date ASC NULLS LAST;

COMMENT ON VIEW public.kitchen_expiry_alerts IS 'View of all active batches with expiry status and days remaining';

-- ── 7. Kitchen dashboard summary ────────────────────────────────────────
...
BEGIN
  ...
  WHERE property_id = p_property_id
    AND status IN ('draft','approved','in_production');
  ...
  FROM public.kitchen_inventory_batches
  ...
  RETURN QUERY SELECT
    COALESCE(v_today_prod, 0),
    COALESCE(v_pending_prod, 0),
    COALESCE(v_inv_value, 0),
    COALESCE(v_low_stock, 0),
    COALESCE(v_expiring, 0),
    COALESCE(v_waste_cost, 0),
    COALESCE(v_avg_fc_pct, 0),
    COALESCE(v_efficiency, 0),
    COALESCE(v_batches_active, 0),
    COALESCE(v_waste_count, 0);
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

COMMENT ON FUNCTION public.kitchen_dashboard_summary(TEXT) IS
  'Returns aggregated dashboard metrics for the kitchen module';

-- ── 8. Process waste (reduce inventory) ─────────────────────────────────
CREATE OR REPLACE FUNCTION public.process_kitchen_waste(
  p_waste_id UUID,
...
DECLARE
  ...
  v_cost NUMERIC := 0;
BEGIN
  SELECT * INTO v_waste FROM public.kitchen_waste WHERE id = p_waste_id;
...
COMMENT ON FUNCTION public.process_kitchen_waste(UUID, TEXT) IS
  'Processes approved waste: deducts inventory and records audit trail';

-- ── 9. Generate production planning suggestions ─────────────────────────
CREATE OR REPLACE FUNCTION public.generate_kitchen_production_plan(
  p_property_id TEXT DEFAULT 'single-property',
...
BEGIN
  -- Get today's occupancy and reservations
  SELECT
  ...
  -- Generate suggestions for active menu item recipes
  RETURN QUERY
  SELECT
    kr.id,
    kr.name,
    ...
    COALESCE(v_reservations, 0)::NUMERIC * 0.5, -- rough estimate: 0.5 portions per guest
    GREATEST(
      COALESCE(kii.min_stock_level, 0) - COALESCE(kii.on_hand_qty, 0),
      COALESCE(v_reservations, 0)::NUMERIC * 0.5 - COALESCE(kii.on_hand_qty, 0),
      0
    )
  FROM public.kitchen_recipes kr
...
COMMENT ON FUNCTION public.generate_kitchen_production_plan(TEXT, DATE) IS
  'Generates production planning suggestions based on occupancy, reservations, and stock levels';

-- ── 10. Trigger: auto-update recipe cost when ingredients change ────────
CREATE OR REPLACE FUNCTION public.trigger_update_kitchen_recipe_cost()
RETURNS TRIGGER AS $$
BEGIN
  PERFORM public.update_kitchen_recipe_cost(NEW.recipe_id);
  RETURN NEW;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

DROP TRIGGER IF EXISTS trg_kitchen_recipe_ingredient_cost_update ON public.kitchen_recipe_ingredients;
CREATE TRIGGER trg_kitchen_recipe_ingredient_cost_update
  AFTER INSERT OR UPDATE OR DELETE ON public.kitchen_recipe_ingredients
  FOR EACH ROW EXECUTE FUNCTION public.trigger_update_kitchen_recipe_cost();
