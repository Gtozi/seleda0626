-- Migration 229: Exclude Out-of-Order/Out-of-Service/Maintenance rooms from
-- create_booking_atomic capacity count
--
-- Previously the availability check counted every room of a type as sellable
-- capacity, including rooms physically unavailable (Out of Order / Out of
-- Service / Maintenance). This could allow bookings to pass the availability
-- gate even when no physical room could be assigned, causing the per-night
-- assignment step to fail or silently leave room_number NULL.
--
-- This migration redefines create_booking_atomic so the capacity count
-- excludes unsellable rooms, matching the per-room assignment query which
-- already excludes 'Out of Order'. The function body is otherwise identical
-- to migration 227 (per-room occupancy support).
...
BEGIN
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    WHERE rt.name = (v_item->>'roomTypeName')
      AND r.status NOT IN ('Out of Order', 'Out of Service', 'Maintenance');
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    BEGIN
      ...
      FOR v_i IN 1..v_qty LOOP
        ...
        -- ── Read per-room occupancy (falls back to item-level) ────────────────
        v_room_adults   := COALESCE((v_item->'occupancies'->(v_i - 1)->>'adults')::INTEGER,
                                    COALESCE((v_item->>'adults')::INTEGER, 1));
        v_room_children := COALESCE((v_item->'occupancies'->(v_i - 1)->>'children')::INTEGER,
