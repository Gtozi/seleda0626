-- Migration 184: Kitchen Management Module — DB Functions
  ...
  FOR v_line IN SELECT * FROM public.kitchen_production_lines WHERE production_order_id = p_production_order_id LOOP
    IF v_line.actual_qty > 0 THEN
      ...
      IF v_inventory_item_id IS NOT NULL THEN
        ...
        v_consumed_cost := 0;
        PERFORM FROM public.consume_kitchen_batch_fefo(
...
DECLARE
  ...
  v_consumed_cost NUMERIC;
BEGIN
  -- Find the kitchen recipe linked to this menu item via menu_item_id
  SELECT id INTO v_recipe_id FROM public.kitchen_recipes
  WHERE menu_item_id = p_menu_item_id AND recipe_type = 'menu_item' AND status = 'active'
  LIMIT 1;
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(v_reservations, 0)::NUMERIC * 0.5,
    GREATEST(
