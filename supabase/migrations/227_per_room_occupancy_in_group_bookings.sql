-- Migration 227: Support per-room occupancy in group bookings
--
-- Previously, create_booking_atomic read a single adults/children pair from
-- each item JSONB and applied it to every room in that item's qty loop.
-- This meant a group booking of 3× Double rooms always had identical occupancy
-- for all 3 rooms.
--
-- This migration updates the function to read an optional `occupancies` array
-- from each item:
--   { "roomTypeName": "Double", "qty": 3, "rate": 150,
--     "occupancies": [
--       { "adults": 2, "children": 0 },
--       { "adults": 1, "children": 1 },
--       { "adults": 3, "children": 0 }
...
--   }
--
-- If the occupancies array is absent or the index is out of bounds, it falls
-- back to the item-level adults/children fields, preserving backward
