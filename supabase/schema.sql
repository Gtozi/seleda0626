...
create table if not exists guests (
  ...
  parent_corporate_id text,
  is_primary_contact boolean not null default false,
  billing_routing_profile_id text
);
...
-- Robustly replace the status check constraint on existing group_bookings tables
do $$
declare
...
begin
  ...
  where constraint_name like 'group_bookings%status%'
  limit 1;

  if v_constraint_name is not null then
    execute format('alter table group_bookings drop constraint %I', v_constraint_name);
  end if;

  alter table group_bookings add constraint group_bookings_status_check check (status in ('Pending', 'Confirmed', 'CheckedIn', 'Completed', 'Cancelled'));
end
$$;
...
select
  'fp_' || extract(year from current_date) || '_' || extract(month from current_date),
  extract(year from current_date)::integer,
  extract(month from current_date)::integer,
  to_char(current_date, 'YYYY Month'),
  date_trunc('month', current_date)::date,
  (date_trunc('month', current_date) + interval '1 month' - interval '1 day')::date
where not exists (select 1 from fiscal_periods where fiscal_year = extract(year from current_date)::integer and period_number = extract(month from current_date)::integer);
...
create table if not exists global_settings (
  ...
  isolation_policy jsonb not null default '{"finance": false, "hr": false, "executive": false, "dualSignature": false}'::jsonb,
...
drop policy if exists "Allow all public reads" on group_bookings;
create policy "Allow all public reads" on group_bookings for select using (true);
drop policy if exists "Allow all public writes" on group_bookings;
create policy "Allow all public writes" on group_bookings for all using (true) with check (true);

drop policy if exists "Allow all public reads" on corporate_accounts;
create policy "Allow all public reads" on corporate_accounts for select using (true);
drop policy if exists "Allow all public writes" on corporate_accounts;
create policy "Allow all public writes" on corporate_accounts for all using (true) with check (true);
...
-- Migrate existing charges from reservations JSON to folio_lines
insert into folio_lines (id, folio_id, line_number, transaction_date, description, amount, quantity, unit_price, line_type, is_voided, created_by, created_at)
select
  gen_random_uuid()::text,
  f.id,
  (row_number() over (partition by f.id order by (c->>'date'))),
  coalesce((c->>'date')::date, current_date),
  c->>'description',
  coalesce((c->>'amount')::numeric, 0.00),
  1,
  coalesce((c->>'amount')::numeric, 0.00),
  coalesce(c->>'type', 'Extra'),
  coalesce((c->>'isVoided')::boolean, false),
  null,
  now()
from reservations r
join folios f on f.reservation_id = r.id
join jsonb_array_elements(r.charges) as c on true
where c->>'description' is not null;

-- Migrate existing payments from reservations JSON to folio_payments
insert into folio_payments (id, folio_id, payment_date, amount, payment_method, reference_number, is_voided, cashier_id, created_by, created_at)
select
  gen_random_uuid()::text,
  f.id,
  now(),
  coalesce((p->>'amount')::numeric, 0.00),
  coalesce(p->>'method', 'Other'),
  p->>'notes',
  coalesce((p->>'isVoided')::boolean, false),
  null,
  null,
  now()
from reservations r
join folios f on f.reservation_id = r.id
join jsonb_array_elements(r.payments) as p on true
...
-- 4. FOREIGN KEY CONSTRAINTS FOR GROUP TABLES

-- Add FK constraints for columns referencing group_profiles
alter table reservations add constraint fk_reservations_group foreign key (group_id) references group_profiles(id) on delete set null;
alter table guests add constraint fk_guests_parent_group foreign key (parent_group_id) references group_profiles(id) on delete set null;
alter table guests add constraint fk_guests_parent_corporate foreign key (parent_corporate_id) references group_profiles(id) on delete set null;
...
create policy system_users_anon_select
  on system_users for select
  to anon
  using (true);

-- custom_roles: anon key can only SELECT
create policy custom_roles_anon_select
...
create policy audit_events_anon_select
