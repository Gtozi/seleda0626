...
export default function GuestAmenitiesModule() {
  const { formatAmount, addNotification, inventoryItems } = useERP();
  
  // Filter only Guest Amenities from global inventory or use local demo data if needed
  // For consistency with typical patterns, let's use a local managed state first
  const [inventory, setInventory] = useState<AmenityItem[]>([
    ...
    { id: 'HK-INV-008', name: 'Dental Kit', category: 'Guest Amenities', unit: 'pcs', stockInHand: 310, reorderLevel: 50, price: 0.50, lastRestock: '2026-05-26' },
    { id: 'HK-INV-009', name: 'Shaving Kit', category: 'Guest Amenities', unit: 'pcs', stockInHand: 240, reorderLevel: 40, price: 0.75, lastRestock: '2026-05-26' },
    { id: 'HK-INV-010', name: 'Vanity Kit', category: 'Guest Amenities', unit: 'pcs', stockInHand: 180, reorderLevel: 30, price: 0.40, lastRestock: '2026-05-26' },
    { id: 'HK-INV-011', name: 'Slippers (Pair)', category: 'Guest Amenities', unit: 'pair', stockInHand: 120, reorderLevel: 30, price: 2.50, lastRestock: '2026-05-22' },
  ]);
  ...
  const [history, setHistory] = useState([
    ...
    { type: 'Issue', item: 'Bath Soap', qty: 50, to: 'Shift Cart B', time: '09:12 AM' },
    { type: 'Issue', item: 'Dental Kit', qty: 20, to: 'VIP Sector', time: '08:30 AM' },
  ]);
  ...
  return (
    <div className="space-y-6 animate-fade-in" id="guest-amenities-console">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-black text-slate-900 dark:text-white uppercase tracking-tight flex items-center gap-2">
            <Sparkles className="text-indigo-500" size={20} />
            Guest Amenities Console
          </h2>
          <p className="text-xs text-slate-500 font-mono italic">Specialized terminal for managing and issuing room amenities to staff.</p>
        </div>
        <div className="flex gap-2">
          <button className="flex items-center gap-2 px-5 py-2.5 bg-slate-100 dark:bg-slate-800 text-slate-900 dark:text-white rounded-2xl text-[10px] font-black uppercase tracking-tight shadow-sm hover:bg-slate-200 transition-all border border-slate-200 dark:border-slate-700">
        ...
        <div className="lg:col-span-8 space-y-4">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-4 rounded-3xl flex flex-wrap items-center justify-between gap-4 shadow-3xs">
            <div className="relative flex-1 min-w-[200px]">
              ...
              <input 
                ...
                className="w-full pl-9 pr-4 py-2.5 bg-slate-50 dark:bg-slate-850 border border-transparent dark:border-slate-800 rounded-xl text-xs outline-none focus:ring-1 focus:ring-indigo-500 font-sans"
              />
            </div>
            <div className="flex items-center gap-2 px-3 py-1.5 bg-slate-100 dark:bg-slate-800 rounded-xl text-[10px] font-bold text-slate-500 uppercase tracking-wider border border-slate-200 dark:border-slate-700">
              <Filter size={12} /> Category: Guest Amenities
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-slate-900/30 border border-slate-200 dark:border-slate-700 rounded-[2rem] overflow-hidden shadow-3xs dark:shadow-slate-900/20">
            <table className="w-full text-left border-collapse font-sans dark:text-slate-200">
              <thead>
                <tr className="bg-slate-50 dark:bg-slate-900/40 font-bold text-slate-400 dark:text-slate-300 text-[10px] uppercase tracking-widest h-12 border-b dark:border-slate-700">
                  <th className="px-6">Amenity Description</th>
                  <th className="px-6 text-center">Unit</th>
                  <th className="px-6 text-center">In Hand</th>
                  <th className="px-6 text-center">Reorder At</th>
        ...
        <div className="lg:col-span-4 space-y-6">
            ...
            <div className="relative z-10 space-y-4">
              <div className="flex justify-between items-start">
                ...
                <div className="flex flex-col items-end">
                   <h4 className="text-2xl font-black tabular-nums">8</h4>
              ...
              <div>
                ...
                <p className="text-[10px] text-slate-400 font-mono uppercase tracking-widest leading-relaxed">Fast-track amenity distribution to shift carts.</p>
              </div>
              <div className="space-y-4 pt-2">
                 <div className="space-y-1">
                    ...
                    <select 
                      ...
                      className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-3 text-xs outline-none focus:ring-1 focus:ring-white/20 text-slate-300 [&>option]:text-slate-900"
                    >
                 ...
                 <div className="space-y-1">
                    <label className="text-[9px] font-black text-slate-400 uppercase">Select Amenity to Issue</label>
                    <select 
                 ...
                 <div className="space-y-1">
                    ...
                    <input 
                      ...
                      className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-3 text-xs outline-none focus:ring-1 focus:ring-white/20 text-slate-300"
                    />
                 </div>
                 <button 
                  onClick={handleIssueItem}
                  className="w-full bg-indigo-600 text-white py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-indigo-500 transition-all mt-2 shadow-lg shadow-indigo-500/20"
                 >
                    Generate Issue Slip
                 </button>
              </div>
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 rounded-[2.5rem] shadow-3xs space-y-4">
            <div className="flex items-center gap-3 border-b dark:border-slate-800 pb-4">
              ...
              <div>
                 <h4 className="text-xs font-black text-slate-900 dark:text-white uppercase">Issue Log History</h4>
                 <p className="text-[9px] text-slate-400 font-mono">Recent amenities distributed.</p>
              </div>
            </div>
