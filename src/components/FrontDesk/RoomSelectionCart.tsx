    ...
    >
      <div className="p-5 border-b border-slate-100 bg-slate-50/50">
        <h3 className="font-semibold text-slate-900 flex items-center gap-2">
          <ShoppingBag size={18} className="text-indigo-500" /> Your Selection
        </h3>
      ...
      <div className="p-5 space-y-4">
        {roomSelections.length === 0 ? (
          <div className="text-center py-8">
            <Bed size={32} className="mx-auto text-slate-300 mb-2" />
            <p className="text-sm text-slate-500 font-medium">No rooms selected yet</p>
            <p className="text-xs text-slate-400 mt-1">Select a room to see your estimate</p>
          </div>
        ) : (
          <div className="space-y-4">
            <div className="space-y-3">
              ...
              {roomSelections.map(selection => {
                ...
                return (
                  <div key={selection.roomType} className="space-y-2 pb-3 border-b border-slate-100 last:border-0 last:pb-0">
                    <div className="flex items-start justify-between gap-3 text-sm">
                      <div>
                        <p className="font-semibold text-slate-900">{rt?.name || selection.roomType}</p>
                        <p className="text-xs text-slate-500">{quantity} room{quantity > 1 ? 's' : ''} × {formatAmount(rate)} / night</p>
                      </div>
                      <span className="font-semibold text-slate-900 shrink-0">{formatAmount(lineTotal)}</span>
                    </div>
                    ...
                    {selection.roomType && (
                      <div className="pt-2">
                        <p className="text-[10px] font-bold text-slate-500 uppercase mb-2">Assign rooms per night</p>
                        <div className="space-y-2 max-h-60 overflow-y-auto pr-1">
                          {Array.from({ length: nights }).map((_, nightIndex) => {
                            ...
                            return (
                              <div key={nightIndex} className="flex items-center gap-2 p-2 rounded-xl border border-slate-100 bg-slate-50/30">
                                <span className="text-[10px] font-bold text-slate-500 w-12 shrink-0">{dateStr}</span>
                                <div className="flex-1">
                                  <div className="flex flex-wrap items-center gap-2">
                                    ...
                                    <button
                                      ...
                                      className={`text-left flex items-center justify-between gap-1 px-2 py-1.5 text-xs rounded-lg border transition focus:outline-none focus:ring-1 focus:ring-indigo-400 ${isAuto ? 'bg-indigo-50 border-indigo-400 text-indigo-800 font-medium' : 'bg-white border-slate-200 text-slate-600 hover:border-indigo-300 hover:bg-slate-50'}`}
                                    >
                                      <span>Auto</span>
                                      {isAuto && <Check size={12} className="text-indigo-600" />}
                                    </button>
                                    {typeRooms.map(r => {
                                      ...
                                      return (
                                        <button
                                          ...
                                          className={`text-center flex items-center justify-center gap-1 px-2 py-1.5 text-xs rounded-lg border transition focus:outline-none focus:ring-1 focus:ring-indigo-400 ${isSelected ? 'bg-indigo-50 border-indigo-400 text-indigo-800 font-medium' : 'bg-white border-slate-200 text-slate-600 hover:border-indigo-300 hover:bg-slate-50'}`}
                                        >
                                          <span>{r.number}</span>
            ...
            <div className="pt-2 border-t border-slate-100 flex justify-between items-baseline">
              <div>
                <p className="text-sm font-bold text-slate-900">Room Estimate</p>
                <p className="text-[10px] text-slate-500">Taxes & add-ons in summary</p>
              </div>
              <span className="text-2xl font-black text-indigo-600">{formatAmount(roomTotalEstimate)}</span>
            </div>
          </div>
        )}
      </div>
