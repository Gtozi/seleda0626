...
export default function TariffSummarySection({
  ...
  return (
    <motion.div
      ...
      className="bg-gradient-to-br from-amber-50 to-orange-50 border border-amber-200/70 rounded-2xl p-5 space-y-4 shadow-sm"
    >
      <div className="flex items-center justify-between border-b border-amber-200/60 pb-3">
        <h4 className="text-sm font-sans font-black text-amber-900 tracking-tight flex items-center gap-2">
          <span className="flex items-center justify-center w-5 h-5 rounded-md bg-amber-200/60 text-amber-700">
            <CreditCard size={12} />
          </span>
          Tariff Finalization
        </h4>
        <div className="text-right">
          <div className="text-[10px] font-mono text-amber-700 uppercase font-bold">Estimated Cost</div>
          <div className="text-xl font-mono font-black text-amber-900">{formatAmount(grandTotal)}</div>
        </div>
      </div>
      ...
      <div className="space-y-2 text-[11px] font-medium text-amber-800">
        {hasMultipleRoomTypes && roomBreakdown ? (
          <div className="space-y-1">
            <div className="text-[10px] font-bold uppercase text-amber-700">Room Revenue ({nights} Night{nights !== 1 ? 's' : ''})</div>
            ...
            <div className="flex justify-between pl-2 pt-1 border-t border-amber-200/30">
        ...
        {nights > 0 && (
          <div className="flex justify-between pt-2 border-t border-amber-200/50">
