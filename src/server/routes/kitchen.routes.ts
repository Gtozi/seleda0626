...
router.get('/dashboard', authenticate, async (req, res) => {
  ...
  try {
    ...
    let todayProdQ = supabaseAdmin
      .from('kitchen_production_orders')
      .select('*', { count: 'exact', head: true })
      .gte('created_at', `${today}T00:00:00Z`)
      .lte('created_at', `${today}T23:59:59Z`);
    if (outlet_id) todayProdQ = todayProdQ.eq('kitchen_id', outlet_id);
    ...
    if (outlet_id) pendingProdQ = pendingProdQ.eq('kitchen_id', outlet_id);
    ...
    let expiringQ = supabaseAdmin
      .from('kitchen_inventory_batches')
      .select('*', { count: 'exact', head: true })
    ...
    let recipeQ = supabaseAdmin
      ...
      .select('food_cost_percent')
      .eq('is_deleted', false)
      .eq('status', 'active');
    if (outlet_id) recipeQ = recipeQ.eq('outlet_id', outlet_id);
    const { data: recipes, error: recipeErr } = await recipeQ;
    if (recipeErr) throw recipeErr;
    const avgFoodCostPercent = recipes && recipes.length
      ? recipes.reduce((sum, r) => sum + Number(r.food_cost_percent || 0), 0) / recipes.length
      : 0;
    ...
    return res.json({
      ...
      avg_food_cost_percent: avgFoodCostPercent,
...
router.get('/storage-locations', authenticate, async (req, res) => {
  if (!hasSupabaseAdminConfig || !supabaseAdmin) return dbNotConfigured(res);
  const { outlet_id } = req.query as Record<string, string>;
  let q = supabaseAdmin.from('kitchen_storage_locations').select('*').order('name');
...
router.post('/storage-locations', authenticate, requirePermission('fb:kitchen:write'), async (req, res) => {
  if (!hasSupabaseAdminConfig || !supabaseAdmin) return dbNotConfigured(res);
  const { data, error } = await supabaseAdmin.from('kitchen_storage_locations').insert(req.body).select().single();
  if (error) return res.status(500).json({ error: error.message });
  return res.status(201).json({ success: true, id: data.id });
});

router.patch('/storage-locations/:id', authenticate, requirePermission('fb:kitchen:write'), async (req, res) => {
  if (!hasSupabaseAdminConfig || !supabaseAdmin) return dbNotConfigured(res);
  const { data, error } = await supabaseAdmin.from('kitchen_storage_locations').update(req.body).eq('id', req.params.id).select().single();
  if (error) return res.status(500).json({ error: error.message });
  return res.json({ success: true, data });
});

// ── Recipes ─────────────────────────────────────────────────────────────
async function calculateKitchenRecipeCost(recipeId: string) {
  if (!supabaseAdmin) return { total_cost: 0, cost_per_portion: 0, food_cost_percent: 0 };
  const { data: recipe } = await supabaseAdmin.from('kitchen_recipes').select('*').eq('id', recipeId).single();
  if (!recipe) return { total_cost: 0, cost_per_portion: 0, food_cost_percent: 0 };
  ...
  const { data: rawItems } = await supabaseAdmin.from('kitchen_inventory_items').select('id, avg_cost').in('id', rawIds.length ? rawIds : ['00000000-0000-0000-0000-000000000000']);
  const { data: subRecipes } = await supabaseAdmin.from('kitchen_recipes').select('id, total_cost, yield_qty').in('id', subRecipeIds.length ? subRecipeIds : ['00000000-0000-0000-0000-000000000000']);
  ...
  const foodCostPercent = sellingPrice ? (costPerPortion / sellingPrice) * 100 : 0;
  return { total_cost: totalCost, cost_per_portion: costPerPortion, food_cost_percent: foodCostPercent };
}

router.get('/recipes', authenticate, async (req, res) => {
  if (!hasSupabaseAdminConfig || !supabaseAdmin) return dbNotConfigured(res);
  const { type, outlet_id } = req.query as Record<string, string>;
  let q = supabaseAdmin.from('kitchen_recipes').select('*, kitchen_recipe_ingredients(*)').eq('is_deleted', false).order('name');
...
router.get('/recipes/:id', authenticate, async (req, res) => {
  ...
  const { data, error } = await supabaseAdmin
    .from('kitchen_recipes')
    .select('*, kitchen_recipe_ingredients(*)')
    .eq('id', req.params.id)
    .single();
  if (error) return res.status(500).json({ error: error.message });
  return res.json(data);
...
router.post('/recipes', authenticate, requirePermission('fb:kitchen:write'), async (req, res) => {
  ...
  const { data: recipe, error } = await supabaseAdmin.from('kitchen_recipes').insert(recipeFields).select().single();
  if (error) return res.status(500).json({ error: error.message });
  if (ingredients && ingredients.length > 0) {
    const lines = ingredients.map((line: any) => ({ ...line, recipe_id: recipe.id }));
    const { error: lineError } = await supabaseAdmin.from('kitchen_recipe_ingredients').insert(lines);
    if (lineError) return res.status(500).json({ error: lineError.message });
  }
  const costs = await calculateKitchenRecipeCost(recipe.id);
  await supabaseAdmin.from('kitchen_recipes').update(costs).eq('id', recipe.id);
...
router.patch('/recipes/:id', authenticate, requirePermission('fb:kitchen:write'), async (req, res) => {
  if (!hasSupabaseAdminConfig || !supabaseAdmin) return dbNotConfigured(res);
  const { ingredients, ...recipeFields } = req.body;
  const { data: recipe, error } = await supabaseAdmin.from('kitchen_recipes').update(recipeFields).eq('id', req.params.id).select().single();
  if (error) return res.status(500).json({ error: error.message });
  if (ingredients) {
    await supabaseAdmin.from('kitchen_recipe_ingredients').delete().eq('recipe_id', req.params.id);
  ...
  const costs = await calculateKitchenRecipeCost(req.params.id);
  await supabaseAdmin.from('kitchen_recipes').update(costs).eq('id', req.params.id);
...
router.delete('/recipes/:id', authenticate, requirePermission('fb:kitchen:write'), async (req, res) => {
  if (!hasSupabaseAdminConfig || !supabaseAdmin) return dbNotConfigured(res);
  const { error } = await supabaseAdmin.from('kitchen_recipes').update({ is_deleted: true }).eq('id', req.params.id);
  if (error) return res.status(500).json({ error: error.message });
...
router.get('/recipes/:id/cost', authenticate, async (req, res) => {
  ...
  try {
    ...
    const rawIds = (lines || []).filter((l: any) => l.ingredient_type === 'raw_material').map((l: any) => l.raw_ingredient_id).filter(Boolean);
    const { data: rawItems } = await supabaseAdmin.from('kitchen_inventory_items').select('id, name, avg_cost, unit').in('id', rawIds.length ? rawIds : ['00000000-0000-0000-0000-000000000000']);
    const rawItemMap = new Map((rawItems || []).map((r: any) => [r.id, r]));
...
router.get('/production-orders', authenticate, async (req, res) => {
  ...
  let q = supabaseAdmin
    .from('kitchen_production_orders')
    .select('*, kitchen_recipes(name, recipe_code), kitchen_production_lines(*)')
    .eq('is_deleted', false)
    .order('created_at', { ascending: false });
  if (status) q = q.eq('status', status);
  if (outlet_id) q = q.eq('kitchen_id', outlet_id);
  const { data, error } = await q;
  if (error) return res.status(500).json({ error: error.message });
  return res.json(data || []);
});

router.post('/production-orders', authenticate, requirePermission('fb:kitchen:write'), async (req, res) => {
  if (!hasSupabaseAdminConfig || !supabaseAdmin) return dbNotConfigured(res);
  const { lines, outlet_id, ...orderFields } = req.body;
  const production_number = generateNumber('KPO');
  const batch_number = orderFields.batch_number || generateNumber('KBATCH');
  const insertData: any = { ...orderFields, production_number, batch_number };
  if (outlet_id) insertData.kitchen_id = outlet_id;
  ...
  if (lines && lines.length > 0) {
    ...
    const { error: lineError } = await supabaseAdmin.from('kitchen_production_lines').insert(productionLines);
    if (lineError) return res.status(500).json({ error: lineError.message });
  }
  return res.status(201).json({ success: true, id: order.id, production_number });
});

router.patch('/production-orders/:id/status', authenticate, requirePermission('fb:kitchen:write'), async (req, res) => {
  if (!hasSupabaseAdminConfig || !supabaseAdmin) return dbNotConfigured(res);
  ...
  const { data, error } = await supabaseAdmin.from('kitchen_production_orders').update(update).eq('id', req.params.id).select().single();
  if (error) return res.status(500).json({ error: error.message });
  return res.json({ success: true, data });
});

router.post('/production-orders/:id/complete', authenticate, requirePermission('fb:kitchen:write'), async (req, res) => {
  if (!hasSupabaseAdminConfig || !supabaseAdmin) return dbNotConfigured(res);
  const { actual_qty, performed_by, labor_cost = 0 } = req.body;
  try {
    const { data: order, error: orderError } = await supabaseAdmin
      .from('kitchen_production_orders')
      .select('*, kitchen_production_lines(*), kitchen_recipes(name, recipe_code)')
      .eq('id', req.params.id)
      .single();
    ...
    const { data: recipeCost } = await supabaseAdmin.from('kitchen_recipes').select('total_cost, cost_per_portion').eq('id', order.recipe_id).single();
    ...
    const batch_number = generateNumber('KBATCH');
    let { data: inventoryItem } = await supabaseAdmin
      ...
      .single();
    if (!inventoryItem) {
      const { data: newItem, error: createItemError } = await supabaseAdmin.from('kitchen_inventory_items').insert({
        name: order.kitchen_recipes?.name || 'Produced Item',
        item_type: 'finished_good',
        unit: order.kitchen_recipes?.yield_unit || 'portion',
        recipe_id: order.recipe_id,
        outlet_id: order.kitchen_id || null,
      }).select().single();
    ...
    const { data: batch, error: batchError } = await supabaseAdmin
      ...
      .insert({
        ...
        chef_id: performed_by || order.chef_id,
    ...
    await supabaseAdmin.from('kitchen_production_orders').update({
      status: 'completed',
...
router.get('/inventory', authenticate, async (req, res) => {
  if (!hasSupabaseAdminConfig || !supabaseAdmin) return dbNotConfigured(res);
  const { type, outlet_id } = req.query as Record<string, string>;
  let q = supabaseAdmin.from('kitchen_inventory_items').select('*').eq('is_deleted', false).order('name');
...
router.post('/inventory', authenticate, requirePermission('fb:kitchen:write'), async (req, res) => {
  if (!hasSupabaseAdminConfig || !supabaseAdmin) return dbNotConfigured(res);
  const { data, error } = await supabaseAdmin.from('kitchen_inventory_items').insert(req.body).select().single();
  if (error) return res.status(500).json({ error: error.message });
  return res.status(201).json({ success: true, id: data.id });
});

router.patch('/inventory/:id', authenticate, requirePermission('fb:kitchen:write'), async (req, res) => {
  if (!hasSupabaseAdminConfig || !supabaseAdmin) return dbNotConfigured(res);
  const { data, error } = await supabaseAdmin.from('kitchen_inventory_items').update(req.body).eq('id', req.params.id).select().single();
  if (error) return res.status(500).json({ error: error.message });
  return res.json({ success: true, data });
});
...
router.get('/batches', authenticate, async (req, res) => {
  ...
  let q = supabaseAdmin
    .from('kitchen_inventory_batches')
    .select('*, kitchen_inventory_items(name, item_type), kitchen_storage_locations(name)')
    .eq('is_deleted', false)
    .order('created_at', { ascending: false });
  if (status) q = q.eq('status', status);
  if (outlet_id) q = q.eq('outlet_id', outlet_id);
...
router.get('/movements', authenticate, async (req, res) => {
  ...
  const { data, error } = await supabaseAdmin
    ...
    .select('*, kitchen_inventory_items(name)')
    .order('created_at', { ascending: false })
    .limit(Number(limit));
  if (error) return res.status(500).json({ error: error.message });
  return res.json(data || []);
...
router.get('/transfers', authenticate, async (req, res) => {
  ...
  const { status, outlet_id } = req.query as Record<string, string>;
  let q = supabaseAdmin
    .from('kitchen_transfers')
    .select('*, kitchen_inventory_items(name), from_location:kitchen_storage_locations!kitchen_transfers_from_location_id_fkey(name), to_location:kitchen_storage_locations!kitchen_transfers_to_location_id_fkey(name)')
    .eq('is_deleted', false)
    .order('created_at', { ascending: false });
  if (status) q = q.eq('status', status);
  if (outlet_id) q = q.eq('outlet_id', outlet_id);
...
router.post('/transfers', authenticate, requirePermission('fb:kitchen:write'), async (req, res) => {
  if (!hasSupabaseAdminConfig || !supabaseAdmin) return dbNotConfigured(res);
  const transfer_number = generateNumber('KTR');
  const { data, error } = await supabaseAdmin.from('kitchen_transfers').insert({ ...req.body, transfer_number }).select().single();
  if (error) return res.status(500).json({ error: error.message });
  return res.status(201).json({ success: true, id: data.id, transfer_number });
});

router.post('/transfers/:id/approve', authenticate, requirePermission('fb:kitchen:write'), async (req, res) => {
  if (!hasSupabaseAdminConfig || !supabaseAdmin) return dbNotConfigured(res);
  const { approved_by } = req.body;
  try {
    const { data: transfer, error } = await supabaseAdmin.from('kitchen_transfers').select('*').eq('id', req.params.id).single();
    ...
    const { data: item } = await supabaseAdmin.from('kitchen_inventory_items').select('avg_cost').eq('id', transfer.inventory_item_id).single();
    const unitCost = Number(item?.avg_cost || 0);
    const totalCost = unitCost * Number(transfer.quantity || 0);

    await supabaseAdmin.from('kitchen_inventory_movements').insert({
...
router.post('/waste', authenticate, requirePermission('fb:kitchen:write'), async (req, res) => {
  if (!hasSupabaseAdminConfig || !supabaseAdmin) return dbNotConfigured(res);
  try {
    const { data: item } = await supabaseAdmin.from('kitchen_inventory_items').select('avg_cost').eq('id', req.body.inventory_item_id).single();
    const costValue = Number(item?.avg_cost || 0) * Number(req.body.quantity || 0);
    const { data, error } = await supabaseAdmin.from('kitchen_waste').insert({ ...req.body, cost_value: costValue }).select().single();
...
router.post('/waste/:id/approve', authenticate, requirePermission('fb:kitchen:write'), async (req, res) => {
  if (!hasSupabaseAdminConfig || !supabaseAdmin) return dbNotConfigured(res);
  const { approved_by } = req.body;
  try {
    const { data: waste, error } = await supabaseAdmin.from('kitchen_waste').select('*').eq('id', req.params.id).single();
    ...
    await supabaseAdmin.from('kitchen_waste').update({ status: 'approved', approved_by, approved_at: new Date().toISOString() }).eq('id', req.params.id);
...
router.get('/expiry-alerts', authenticate, async (req, res) => {
  ...
  const { status, outlet_id } = req.query as Record<string, string>;
  try {
    const { data: settings } = await supabaseAdmin.from('kitchen_settings').select('expiry_alert_days, critical_expiry_days').single();
    ...
    const alerts = (data || []).map((batch: any) => {
      ...
      return {
        ...
        item_name: batch.kitchen_inventory_items?.name,
        item_type: batch.kitchen_inventory_items?.item_type,
        ...
        storage_location_name: batch.kitchen_storage_locations?.name,
...
router.get('/production-planning', authenticate, async (req, res) => {
  ...
  try {
    let recipeQ = supabaseAdmin
      .from('kitchen_recipes')
      .select('id, name, yield_qty, yield_unit, cost_per_portion, selling_price, food_cost_percent')
      .eq('is_deleted', false)
      .eq('status', 'active');
    if (outlet_id) recipeQ = recipeQ.eq('outlet_id', outlet_id);
...
router.get('/settings', authenticate, async (_req, res) => {
  if (!hasSupabaseAdminConfig || !supabaseAdmin) return dbNotConfigured(res);
  const { data, error } = await supabaseAdmin.from('kitchen_settings').select('*').single();
  if (error) return res.status(500).json({ error: error.message });
  return res.json(data);
});

router.patch('/settings', authenticate, requirePermission('fb:kitchen:write'), async (req, res) => {
  if (!hasSupabaseAdminConfig || !supabaseAdmin) return dbNotConfigured(res);
  const { data: existing } = await supabaseAdmin.from('kitchen_settings').select('id').single();
  let result;
  if (existing) {
    result = await supabaseAdmin.from('kitchen_settings').update(req.body).eq('id', existing.id).select().single();
  } else {
    result = await supabaseAdmin.from('kitchen_settings').insert(req.body).select().single();
  }
...
router.get('/audit-log', authenticate, async (req, res) => {
  ...
  let q = supabaseAdmin.from('kitchen_audit_log').select('*').order('created_at', { ascending: false }).limit(Number(limit));
